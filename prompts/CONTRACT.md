...
`state/runs/<DATE>/article.json` —— 用今天的目标词（**今天要记的新词 + 到期复习词**，同一个文件里）写成**一篇连贯的简单英文短文**。

## 执行步骤（严格按顺序）

1. 读 `state/runs/<DATE>/due.json`（外层脚本已生成：今天的词，含 `src` 字段 —— `new`=第一次学的新词，`review`=FSRS 到期复习词；每词带词性、短释义、例句）
...
**不要**打印、**不要**标记掌握（known）、**不要**记录「学了一遍」（FSRS 卡）、**不要**上传 OSS、**不要**运行 `run.sh`。这些是外层脚本的活。

> 口径（重要）：「打印出来 = 这个词**学了一遍**」，**不是**掌握。所以打印成功后由 `bin/study_record.py` 写 FSRS 卡片进复习调度（之后按算法自动推到期复习），而不是标 `known`（标 known 会被 App 从新词池永久排除并删掉 FSRS 卡）。
...
- **每个目标词至少出现一次**（新词和到期复习词一视同仁），可用任何屈折形式（-s / -ed / -ing / 不规则形），自然嵌入句子
- 新词需要足够语境让人看懂（第一次见面）；到期复习词要自然重现，别堆成词表
- **A2–B1 简单英语**：句子短、用词基础、一个句子只讲一件事；读不懂就失败
- **4–7 段、每段 3–5 句**；全文是一个连贯的故事或说明（时间线 / 因果），**不是逐词串句**
- `title`：简单英文标题，**不带日期**、不带「复习 / 新词 / 今日」等元信息
- `slug`：标题的小写连字符形式（ASCII）
- `glosses`：覆盖**全部**目标词，每个 2–5 个汉字，取该词**在本篇语境中**的意思
- 不写任何说明性前言（禁止「今天要记 N 个词」这类文字）；除标题外不要其他 heading；不要加词表（脚本会加）
